...
START_TEST(test_memcpy_7) {
  ...
  char src[n];
  s21_memset(src, 0, n);
  s21_memcpy(src, "Test", 4);
  char dest[n], dest_etalon[n];
  char *result = (char *)s21_memcpy(dest, src, n - 1);
  char *result_etalon = (char *)memcpy(dest_etalon, src, n - 1);
  ck_assert_mem_eq(dest, dest_etalon, n - 1);
  ck_assert_mem_eq(result, result_etalon, n - 1);
}
END_TEST
...
START_TEST(test_memcpy_14) {
  ...
  char src[n];
  s21_memset(src, 0, n);
  s21_memcpy(src, "Test", 5);
  char dest[n];
  char dest_etalon[n];
  char *result = (char *)s21_memcpy(dest, src, n);
  memcpy(dest_etalon, src, n);  // ✅ ДОБАВЬ ЭТУ СТРОКУ
  ck_assert_mem_eq(dest, dest_etalon, n);
...
START_TEST(test_memcpy_18) {
  s21_size_t n = 100;
  char src[n];
  s21_memset(src, 'A', n);  // заполняем безопасным символом
  char dest[n];
  char dest_etalon[n];
  char *result = (char *)s21_memcpy(dest, src, n);
  memcpy(dest_etalon, src, n);  // ✅ ДОБАВЬ ЭТУ СТРОКУ
